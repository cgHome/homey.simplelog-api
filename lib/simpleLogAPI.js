...
module.exports = class SimpleLogAPI extends SimpleClass {
  ...
  async #connect() {
    ...
    if (await this.#simpleLogAPI.getInstalled()) {
      this.#connected = true;
      this.#put('appConnected', { id: this.homey.manifest.id, name: this.homey.manifest.name.en || this.homey.manifest.name });
    } else {
  ...
  async #disconnect() {
    ...
    this.#put('appDisconnected', { id: this.homey.manifest.id });
