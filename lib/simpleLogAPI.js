...
module.exports = class SimpleLogAPI extends SimpleClass {
  ...
  async #connect() {
    ...
    let fix = await this.#simpleLogAPI.getInstalled();
    if (typeof fix !== 'boolean') fix = true;

    // if (this.#simpleLogAPI.getInstalled()) {
    if (fix) {
      this.#connected = true;
      this.#put('addapp', { appId: this.homey.manifest.id, appName: this.homey.manifest.name.en });
    } else {
